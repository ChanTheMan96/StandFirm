...
export class MensHelpComponent implements OnInit, OnDestroy {
  ...
  versions = ['KJV', 'NIV', 'ESV', 'NASB', 'NKJV'];
  ...
  selectEmotion(emotion: string) {
    ...
    this.selectedEmotionData = this.emotions.find(e => e.emotion === emotion) || null;
  ...
  findRelevantVerses() {
    ...
    const selected = this.getRandomVerses(this.selectedEmotionData.keywordVerses, 4);
    const requests = selected.map(ref => this.bibleApiService.getPassage(ref));

    forkJoin(requests).pipe(takeUntil(this.destroy$)).subscribe({
      next: results => {
        this.selectedEmotionData!.relevantVerses = results.map((r, i) => ({ reference: `${selected[i]} (${this.selectedVersion})`, text: r.text || 'Text unavailable' }));
        this.loadingVerses = false;
      },
      error: err => {
        console.error(err);
        this.selectedEmotionData!.relevantVerses = selected.map(s => ({ reference: `${s} (${this.selectedVersion})`, text: 'Unable to load verse text.' }));
        this.loadingVerses = false;
      }
    });
  }
